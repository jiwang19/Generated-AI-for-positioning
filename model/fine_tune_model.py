...
class ModelVAE(nn.Module):
    def __init__(self,
        ...
        self.fc_pre1=nn.Linear(468*45,2)
        #self.fc_pre2=nn.Linear(2048,2)

        # # Build Decoder
        # modules = []
        #
        self.decoder_inputt = nn.Linear(latent_dim, 512*45)
        self.sigmoid=nn.Sigmoid()
        # self.de_lstm = nn.LSTM(input_size=512, hidden_size=768,batch_first=True)
        self.de_lstm2 = nn.LSTM(input_size=512, hidden_size=468,num_layers=1, batch_first=True)
        self.relu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        # with torch.no_grad():
            ...
            result = self.decoder_inputt(mu)
            # result=self.sigmoid(result)
            result = result.view(-1, 45,512)
            result, _ = self.de_lstm2(result)
            result=self.Flatten(result)
            output=self.fc_pre1(result)
            # output=self.fc_pre2(output)
